...
class MATHEvaluator:
    ...
    def extract_answer(self, text: str) -> Optional[str]:
        ...
        # handle non-numeric after ####
        match = re.search(r"####\s*([^\n]+)", text)
        if match:
            candidate = match.group(1).strip()
            candidate = re.split(r"(?:Human:|Q:|User:)", candidate)[0].strip()
            token_match = re.match(r"^([-+]?\d*\.?\d+)", candidate)
            if token_match:
        ...
        if lines:
            ...
            if (
                last_line
                and not last_line.startswith("Step")
                and not last_line.startswith("Let")
            ):
                return last_line
    ...
    def evaluate_single_problem(
        self, problem_data: Dict[str, Any], prompt_builder: PromptBuilder
    ) -> Dict[str, Any]:
    ...
    def evaluate_batch(
        ...
        """Evaluate a batch of MATH problems."""
        try:
            # Extract problem information
            problem = [p["problem"] for p in problem_data]
            solution = [p["solution"] for p in problem_data]
            level = [p["level"] for p in problem_data]
            problem_type = [p["type"] for p in problem_data]
...
def main():
    ...
    parser.add_argument(
        "--dataset_path",
        type=str,
        default=f"/shared/data2/jashrp2/query_conditioned_guidelines/data/math/test.parquet",
    ...
    parser.add_argument(
        "--output_path", type=str, help="Path to save evaluation results (JSON file)"
    )
    parser.add_argument(
        "--max_samples",
        type=int,
    ...
    results = evaluator.evaluate_dataset(
        args.dataset_path, prompt_template=args.prompt_template, max_samples=args.max_samples, batch_size=args.batch_size,
